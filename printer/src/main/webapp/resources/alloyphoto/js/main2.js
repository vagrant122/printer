(function() {
	var Main = {
		...
		eventAtt : function() {
			...
			/*
			 * this.addEvent(".pic","dragstart",function(e){ var dx = e.offsetX ?
			 * e.offsetX : e.layerX; var dy = e.offsetY ? e.offsetY : e.layerY;
			 * e.dataTransfer.setData("dx", dx); e.dataTransfer.setData("dy",
			 * dy); });
			 * 
			 * this.addEvent(".pic","drag",function(e){ e.preventDefault(); });
			 * this.addEvent(".picWrapper", "dragover", function(e){
			 * e.preventDefault(); }); this.addEvent(".picWrapper", "drop",
			 * function(e){ var dx = e.dataTransfer.getData("dx"); var dy =
			 * e.dataTransfer.getData("dy"); var x = e.offsetX ? e.offsetX :
			 * e.layerX; var y = e.offsetY ? e.offsetY : e.layerY;
			 * 
			 * var pic = document.getElementById("pic");
			 * 
			 * pic.style.left = (x - dx) + "px"; pic.style.top = (y - dy) +
			 * "px";
			 * 
			 * 
			 * });
			...
			this.addEvent(".pic", "mousedown", function(e) {
				/*
				 * dx = e.offsetX ? e.offsetX : e.layerX; dy = e.offsetY ?
				 * e.offsetY : e.layerY;
				 */
			...
			document.getElementById("picWrapper").onmousemove = function(e) {
				...
				if (clickFlag) {
					...
					/*
					 * var x = e.offsetX ? e.offsetX : e.layerX; var y =
			...
			this.addEvent(".imgWrapper", "click", function(e) {
				...
				if (text == "原图")
					...
					setTimeout(function() {
						var t = +new Date();
						AP.ps(text).replace(img).complete(
								function() {
									console.log(text + "：" + (+new Date() - t)
											/ 1000 + "s");
		...
		initView : function() {
			var func = function() {
				/*
				 * var computedStyle =
				 * getComputedStyle(document.getElementById("picWrapper"));
				 */
		...
		showModel : function() {
			...
			var effectModel = '<li class="e_item"><div class="imgWrapper"><img src="'
					+ context
					+ '/resources/style/image/demo/{pic}.png" alt="" />{effect}</div></li>';
			var html = '<li class="e_item"><div class="imgWrapper"><img src="'
					+ context
					+ '/resources/style/image/demo/e1.jpg" alt="" />原图</div></li>';
			for ( var i in EasyReflection) {
				html += effectModel.replace("{effect}",
						i.length < 3 ? i + "效果" : i).replace("{pic}",
						EasyReflection[i]);
			}
